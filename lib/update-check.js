...
// OSC 9 desktop-notification sequence: ESC ] 9 ; <text> BEL. Claude Code only
// forwards the notification/title OSCs (0/1/2/9/99/777) + BEL, so we strip every
// control char (incl. ESC/BEL) from the payload — that keeps us inside the
// allowlist and prevents the text from smuggling in a second, disallowed sequence.
function osc9(text) {
  const safe = String(text).replace(CONTROL_CHARS, ' ').trim();
  return ESC + ']9;' + safe + BEL;
}
...
// inline message with an OSC 9 desktop notification so it surfaces beyond the
// muted "says:" line. Returns null when up to date.
function noticeFreshParts(currentVersion, opts = {}) {
  ...
  const terminalSequence = osc9(`global-brain ${latest} available — npm i -g ${PKG}@latest`);
...
module.exports = { notice, noticeFresh, noticeFreshParts, refresh, refreshNow, osc9, _cmp: cmp };
